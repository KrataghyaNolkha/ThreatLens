// src/components/CyberRAGLanding.jsx
...
const CounterStat = ({ end, suffix = '', label }) => {
  ...
  return (
    <Box ref={ref} sx={{ textAlign: 'center', py: 5, px: 2 }}>
      <Typography sx={{ fontFamily: '"Helvetica Neue",sans-serif', fontSize: { xs: '2.8rem', md: '4rem' }, fontWeight: 900, letterSpacing: '-0.04em', lineHeight: 1, color: colors.text }}>
        {count}{suffix}
      </Typography>
      <Typography sx={{ fontFamily: '"JetBrains Mono",monospace', fontSize: '0.62rem', color: colors.muted, textTransform: 'uppercase', letterSpacing: '0.12em', mt: 1.5 }}>
        {label}
      </Typography>
    </Box>
  );
...
const HoverGlitch = ({ text, sx = {}, noWrap = true }) => {
  ...
  return (
    <Box
      ...
      sx={{ cursor: 'default', display: 'inline', userSelect: 'none', whiteSpace: noWrap ? 'nowrap' : 'normal', ...sx }}
    >
      {chars.map((ch, i) => (
        <Box
          key={i}
...
const Navbar = () => {
  ...
  return (
    <Box sx={{
      ...
      px: { xs: 2.5, md: 5 },
      py: { xs: 2, md: 3 },
      ...(scrolled && { bgcolor: 'rgba(20,20,20,0.9)', backdropFilter: 'blur(20px)', borderBottom: `1px solid ${colors.border}` })
    }}>
...
const BrutalistBento = () => (
  <Grid container spacing={0} sx={{ borderTop: `2px solid ${colors.border}`, borderLeft: `2px solid ${colors.border}` }}>
    {MODULES.map((module, index) => (
      <Grid item xs={12} sm={6} lg={4} key={module.id} sx={{ borderRight: `2px solid ${colors.border}`, borderBottom: `2px solid ${colors.border}` }}>
        <Box sx={{ minHeight: { xs: 260, lg: 440 }, p: { xs: 3, md: 4 }, display: 'flex', flexDirection: 'column', justifyContent: 'flex-start', bgcolor: index % 2 ? colors.surface : colors.bg }}>
          <Box>
            ...
            <Typography sx={{
              fontFamily: headingFont,
              fontSize: { xs: '2rem', md: 'clamp(2.15rem, 3vw, 3.8rem)', lg: 'clamp(2.15rem, 2.6vw, 3.15rem)' },
              fontWeight: 900,
              textTransform: 'uppercase',
              lineHeight: 0.95,
              ...
              <HoverGlitch text={module.label} noWrap={false} />
            </Typography>
          </Box>
          <Box sx={{ mt: { xs: 4, lg: 8 } }}>
...
const ArchDisplay = () => (
  <Box sx={{ position: 'sticky', top: 120, minHeight: 560, border: `2px solid ${colors.border}`, overflow: 'hidden', bgcolor: colors.surface }}>
  ...
  return (
    <Box sx={{ bgcolor: colors.bg, color: colors.text, minHeight: '100vh', overflowX: 'hidden', fontFamily: bodyFont, position: 'relative' }}>
      ...
      <Box sx={{
        ...
        px: { xs: 2.5, md: 5 },
        pt: { xs: 12, md: 13 },
        pb: { xs: 7, md: 'clamp(4.5rem, 9vh, 7rem)' }
      }}>
        <Box sx={{ position: 'absolute', inset: 0, zIndex: 0 }}>
          <img src="/assets/images/hero_img1.png" alt="ThreatLens command interface" style={{ width: '100%', height: '100%', objectFit: 'cover', filter: 'grayscale(100%) brightness(38%) contrast(118%)' }} />
          <Box sx={{ position: 'absolute', inset: 0, background: 'linear-gradient(90deg, rgba(0,0,0,0.9) 0%, rgba(0,0,0,0.58) 48%, rgba(0,0,0,0.3) 100%)' }} />
          <Box sx={{ position: 'absolute', inset: 0, background: 'radial-gradient(circle at 70% 30%, rgba(255,255,255,0.08), transparent 35%)' }} />
        </Box>

        <Container maxWidth={false} sx={{ position: 'relative', zIndex: 2, px: 0, width: '100%' }}>
          <motion.div initial={{ opacity: 0, y: 40 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 1, ease: [0.16, 1, 0.3, 1] }}>
            <Typography component="h1" sx={{
              fontFamily: headingFont,
              fontSize: {
                xs: 'clamp(2.85rem, 12vw, 4.35rem)',
                sm: 'clamp(4.75rem, 11vw, 6.6rem)',
                md: 'clamp(6.5rem, 8.6vw, 8.9rem)',
                xl: 'clamp(7rem, 7.8vw, 9.6rem)'
              },
              fontWeight: 900,
              lineHeight: { xs: 0.92, md: 0.86 },
              letterSpacing: { xs: '-0.035em', md: '-0.052em' },
              textTransform: 'uppercase',
              color: colors.text,
              maxWidth: '100%',
              overflowWrap: 'normal',
          ...
          <Box sx={{ mt: { xs: 5, md: 7 }, display: 'grid', gridTemplateColumns: { xs: '1fr', md: 'minmax(360px, 620px) auto' }, gap: { xs: 3, md: 5 }, alignItems: 'end' }}>
            <Typography sx={{ fontFamily: monoFont, color: colors.muted, fontSize: { xs: '0.74rem', md: '0.9rem' }, textTransform: 'uppercase', letterSpacing: '0.08em', lineHeight: 1.75, maxWidth: 620 }}>
              // ThreatLens dynamically structures unknown telemetry without rigid regex parsers. Real-time correlation engine active.
            </Typography>
            <Box sx={{ display: 'flex', gap: 2, flexWrap: 'wrap', justifyContent: { xs: 'flex-start', md: 'flex-end' }, '& > button': { flex: { xs: '1 1 140px', sm: '0 0 auto' } } }}>
      ...
      <Box sx={{ py: 15, bgcolor: colors.surface, position: 'relative', overflow: 'hidden' }} id="architecture">
          ...
          <div className="top-ticker-track">
            {[
              ...
              <Box key={i} sx={{ display: 'flex', alignItems: 'center', flexShrink: 0, px: 3 }}>
                <Box sx={{ mr: 2, width: 5, height: 5, borderRadius: '50%', bgcolor: colors.text, flexShrink: 0, opacity: 0.6 }} />
